...
export const SettingsView: React.FC<SettingsViewProps> = ({ 
  ...
  const handleSaveVehicle = () => {
    if(editingVehicle.model) {
        if (editingVehicle.id) {
            // Update existing
            onUpdateVehicles(vehicles.map(v => v.id === editingVehicle.id ? { ...v, ...editingVehicle } as Vehicle : v));
        } else {
            // Create new
  ...
  if (subView === 'vehicle_edit') {
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-32 pt-4 md:pt-4 animate-fade-in transition-colors duration-300">
                            ...
                            {newPaymentType === 'paypal' ? (
                                ...
                                <>
                                    ...
                                    <div className="flex space-x-3">
                                        <input type="text" placeholder="MM/YY" className="w-[120px] flex-shrink-0 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 transition-all dark:text-white" value={cardExpiry} onChange={e => setCardExpiry(e.target.value)} />
                                        <input type="text" placeholder="Inhaber" className="flex-1 min-w-0 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 transition-all dark:text-white" value={cardHolder} onChange={e => setCardHolder(e.target.value)} />
                                    </div>
                                </>
                            )}
                        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-32 md:pt-6 transition-colors duration-300 relative">
      {deletingVehicleId && (
      ...
      <div className="max-w-2xl mx-auto md:p-6">
        ...
        <div className="bg-white dark:bg-gray-800 md:rounded-xl shadow-sm p-4 mb-6 mx-4 md:mx-0 rounded-lg transition-colors">
            ...
            {vehicles.map(v => (
                <div key={v.id} className="flex items-center justify-between p-3 border border-gray-100 dark:border-gray-700 rounded-lg mb-3 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                onClick={() => { setEditingVehicle(v); onNavigate('vehicle_edit'); }}>
